import { getElementLineIntersection } from '../../layout/LayoutUtil';
...
export default function Connect(eventBus, dragging, modeling, rules, canvas, graphicsFactory) {
    ...
    function crop(start, end, source, target) {
        var sourcePath = graphicsFactory.getShapePath(source), targetPath = target && graphicsFactory.getShapePath(target), connectionPath = graphicsFactory.getConnectionPath({ waypoints: [start, end] });
    ...
    eventBus.on('connect.move', function (event) {
        var context = event.context, source = context.source, target = context.target, visual = context.visual, sourcePosition = context.sourcePosition, endPosition, waypoints;
    ...
    eventBus.on('connect.start', function (event) {
        var context = event.context, visual;
