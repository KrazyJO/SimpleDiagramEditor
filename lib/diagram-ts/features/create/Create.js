...
var Create = /** @class */ (function () {
    function Create(eventBus, dragging, rules, modeling, canvas, styles, graphicsFactory) {
        ...
        eventBus.on('create.end', function (event) {
            var context = event.context, source = context.source, shape = context.shape, target = context.target, canExecute = context.canExecute, attach = canExecute && canExecute.attach, connect = canExecute && canExecute.connect, position = {
                x: event.x,
                y: event.y
            };
            if (!canExecute) {
