...
function getLength(point: Point) {
...
export default class Dragging {
	...
	constructor(eventBus: EventBus, canvas: Canvas, selection: Selection) {
		...
		function toLocalPoint(globalPosition: any) {
		...
		function fire(type: any, dragContext?: any): boolean {
			...
			return <boolean>eventBus.fire(dragContext.prefix + '.' + type, event);
		}

		// event listeners

		function move(event: any, activate?: any) {
		...
		function checkCancel(event: any) {
		...
		function trapClickAndEnd(event: any) {
		 ...
		 */
		function init(event: MouseEvent | TouchEvent, relativeTo: Point, prefix: string, options: any) {
